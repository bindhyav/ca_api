...
def test_search_all_events_with_wait():
    ...
    while datetime.now(ev.IST) < deadline and missing_events:
        ...
        for name in list(missing_events):
            ...
            if matches:
                ...
                # --- check 'details' field (case-insensitive keys support) ---
                details_value = None
                # try common keys
                if "details" in matches[0]:
                    ...
                    # fallback: try to find any key that looks like details (case-insensitive)
                    for k, v in matches[0].items():
                        if isinstance(k, str) and k.lower() == "details":
                            details_value = v
                            break

                # normalize and check emptiness
                if details_value is None:
                    print(f"⚠️ Details field MISSING for event: {name}")
                    details_empty_events.append(f"{name} (missing)")
                else:
                    # if it's not a string, stringify for emptiness check
                    try:
                    ...
                    if detail_str == "":
                        print(f"⚠️ Details field EMPTY for event: {name}")
    ...
    if details_empty_events:
        print(f"⚠️ Events with missing/empty details: {details_empty_events}")
        # Optional: for debugging, show similar entries from last_page
    ...
    # Final assert: fail only if events from the original list are still missing
    assert not missing_events, f"Missing events after polling: {missing_events}"
